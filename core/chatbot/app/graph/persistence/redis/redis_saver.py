...
class RedisSaver(BaseCheckpointSaver):
    ...
    def put(
        ...
        versions: Any  # Add this line to accept the new argument
    ) -> RunnableConfig:
            ...
            with _get_sync_connection(self.sync_connection) as conn:
                conn.hset(
                    ...
                    mapping={
                        ...
                        "versions": self.serde.dumps(versions)  # Add this line to store versions
                    },
                )
                logger.info(
    ...
    def put_writes(
        ...
        # Ensure at least one required field is in writes
        required_fields = {'question', 'generation', 'documents'}
        write_keys = {write_key for write_key, _ in writes}
        if not required_fields.intersection(write_keys):
            raise InvalidUpdateError(
                f"Must write to at least one of {list(required_fields)}"
            )

        try:
            with _get_sync_connection(self.sync_connection) as conn:
                for write_key, write_value in writes:
                    conn.hset(key, write_key, self.serde.dumps(write_value))
                logger.info(f"Writes stored successfully for task_id: {task_id}")
        except Exception as e:
    ...
    async def aput_writes(
        ...
        try:
            async with _get_async_connection(self.async_connection) as conn:
                for write_key, write_value in writes:
                    await conn.hset(key, write_key, self.serde.dumps(write_value))
                logger.info(f"Writes stored successfully for task_id: {task_id}")
        except Exception as e:
            logger.error(f"Failed to aput writes: {e}")
            raise
